    ...
    def get_days_present(self, emp_id, month):
        ...
        try:
            ...
            if result and result['effective_days'] is not None:
                ...
                print(f"   ✅ Returning: {days} days (type: {type(days)})")
                return days
            else:
...
class Payroll:
    ...
    def calculate_salary(self, emp_id, month):
        ...
        - Basic Salary: Per day rate × days worked
        - HRA (40%): Proportional to days worked
        - Conveyance (₹1,600): Proportional to days worked
        
        Deductions (all per-day basis):
        - PF: 4.8% of earned basic
        - PT: ₹200/month (proportional)
        - Tax: 2.5% of earned basic
        
        Example: ₹40,000 base, 20/30 days worked
        → Earnings: ₹38,400
        → Deductions: ₹2,080
        → Net: ₹36,320
        """
        ...
        # 3. CONVEYANCE (₹1,600 per month, per day)
        ...
        # 2. PT (Professional Tax) - ₹200 per month (proportional)
